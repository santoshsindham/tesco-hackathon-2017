...
(function ($) {
    ...
    var methods = {
        ...
        init: function (options, element) {
            ...
            if (!elements.ResetButton.hasClass('disabled')) {
                elements.ResetButton.click(function (e) {
                    ...
                    if (state.updating === true) { return; }    // tesco modification
            ...
            $(window).resize(function () {
                ...
                if (settings.prevWindowWidth === $(this).width()) { return; }
                settings.prevWindowWidth = $(this).width();

                // Allow resize if width has actually changed.
                if (elements.CanvasContainer.width() === lastWidth) { return; }
        ...
        ShowPersonalisation: function () {
            ...
            // Resize Canvases
            elements.Canvases.each(function () {
                if (elements.MainContainer.parent().attr('id') === 'lightbox') {    // tesco modification
                    methods.ResizeCanvas($(this));
                }
            });
                            ...
                            if (error == '') {
                                ...
                                if (dropzone.getUploadingFiles().length === 0 && dropzone.getQueuedFiles().length === 0) {
                                    ...
                                    var //rotateLeft = $('<a class="' + settings.Prefix + 'RotateButton" href="#" title="Rotate Left"><i class="fa fa-rotate-left fa-lg"></i></a>'),
                                        ...
                                        cropButton = $('<a class="saveCropBtn ' + settings.Prefix + 'CropButton ' + settings.Prefix + 'Btn" href="#">' + EmaginationJS_language.CropSaveButtonText + '</a>'),    // tesco modification
                                            ...
                                            if (!cropping) {
                                                ...
                                                $.post(settings.APIUrl + '/CropImage', {
                                                    ...
                                                    if (error == '') {
                                                        ...
                                                        image.trigger('click');    // tesco modification
                                                    } else {
        ...
        ClonePreview: function (guid, skuid, callback) {
            ...
            .always(function () {
                if (typeof callback === 'function') {
                    callback({error: error == '' ? null : error, guid: clonedGuid});
                }
            });
        },
                    ...
                    if (error == '') {
                        ...
                        canvas.find(elements.PreviewImages).one('load', function() {
                            ...
                            if (settings.LayerUpdated || settings.TextLayersUpdated) {    // tesco modification
                                ...
                                if (settings.TextLayersUpdated) {
                                    if (typeof settings.onTextLayersUpdated === "function") {
                                        settings.onTextLayersUpdated(settings.TextLayersUpdated);    // tesco modification
                                    }
                                }
