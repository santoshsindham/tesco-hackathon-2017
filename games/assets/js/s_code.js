...
var s_account="tescodevsandpit1"
...
s.linkInternalFilters="javascript:,.tesco.com"
...
d = new Date(); //get the date and time
...
// Placeholder for extra Search Engines (Non standard ones)
// s._extraSearchEngines="zing.com|q|Microsoft Zing>search.comcast.net|q|Comcast.net";

// Channel by domains.
s._channelDomain="Social Networks|facebook.com,twitter.com,digg.com,linkedin.com,myspace.com>Comparison Shopping Engine|pricegrabber.com,shopping.com,nextag.com";

// Channel by pattern
s._channelPattern="PPC|ppc_>Email|eml_>Media|med_>Affiliates|aff_>Price Comparison|pcp_"
...
s.useCommerce=false
s.varUsed="prop12" //Note that this does not use the object s
s.eventList="" //Abandon,Success,Error

//s.variableProvider =
//‘DFA#337209:v3=[["DFA:"+lis+":"+lip+":"+lastimp+":"+lastimptime+":"+lcs+":"+lcp+
//":"+lastclk+":"+lastclktime]]’;
...
function s_doPlugins(s) {
  /* Add calls to plugins here */
                ...
                var vid = s.c_r("OneAppUVI+.tesco.com+%252F");
                if (vid) {
...
//	s.allowOncePerVisit("dfa","variableProvider");

	// Endeca code
	// Below three line are commented for defect 32139 as per David's requirement for omniture
	/*s.integrateSearch("search:eVar17"); //Places search in eVar1 s.integrateSearch("search:eVar1,searchtype:eVar2");
	var a=s.split(s.events,',');
	var e=''; for(var i = 0; i < a.length; i++ ){ if(a[i] == "event1") continue; else e += a[i]?a[i]+',':a[i]; } s.events=e.substring(0,e.length-1);
*/
//DREAM Mail
	s.eVar18=s.getQueryParam("EP_RID"); //places query param value from _rid in eVar18 variable

// Strong mail
	s.eVar19=s.getQueryParam("_rid"); //places query param value from _rid in eVar18 variable

function ratingsDisplayed(totalReviewsCount, avgRating, ratingsOnlyReviewCount, buyAgainPercentage, productID)
	{
		omnitureObjectName='s';
		omnitureReview=new Object();
		var bvo=omnitureReview;	bvo.totalReviewCount=totalReviewsCount;
	...
	s.omnitureReviews("totalReviewCount:eVar21,avgRating:eVar20,type|product:campaign",true);

  /* Set Page View Event */
  s.events=s.apl(s.events,'event10',',',2)

 // get previous value for s.pageName variable, set to prop5 on every page
   s.prop15=s.getPreviousValue(s.pageName,'gpv_p15','');

    /* handle multiple campaign touchpoints */
  s.eVar26="D=c0"; // like s.campaign, Allocation: First
  s.eVar30="D=c0"; // like s.campaign, Allocation: Last

  // get internal campaign tracking code
  if(!s.eVar2)
    s.eVar2=s.getQueryParam('icid');

  // lower case the internal search term
  if(s.eVar1)
 ...
 /* Set Time Parting Variables - GMT */
  s.prop30=s.getTimeParting('h',0); // set hour
  s.prop31=s.getTimeParting('d',0); // set day of week
  s.prop32=s.getTimeParting('w',0); // set weekday/weekend
  ...
  /* copy purchaseID into eVar for data verification */
  if(s.purchaseID) s.eVar10=s.purchaseID;

  /* persist s.prop7 (New/Repeat Customer) if set for 365 days */
  s.prop7=s.getAndPersistValue(s.prop7,'s_prop7_persist',365);

  // set stacked eVar as usual in s_doPlugins
  s.eVar31 = s.crossVisitParticipation(s.campaign,'s_ev31','30','5','>');

// assign touch point eVars in s_doPlugins
  var arry = new Array();
  var c = s.c_r('s_ev31');
  if (c && c != '') {
    arry = eval(c);
    var l = arry.length;
    if (2 < l) {
      s.eVar29 = s.uj(arry,l - 2); // second latest tp
      if (3 < l) {
        s.eVar28 = s.uj(arry,l - 3); // 3rd latest tp
        if (4 < l) {
          s.eVar27 = s.uj(arry,l - 4); // 4th latest tp
        }
      }
    }
  }

  // copy values from traffic to conversion variables
  if(!s.eVar7&&s.prop7) s.eVar7="D=c7";
  if(!s.eVar25&&s.prop13) s.eVar25="D=c13";
  if(!s.eVar15&&s.prop15) s.eVar15="D=c15";
  if(!s.eVar16&&s.prop16) s.eVar16="D=c16";
  if(!s.eVar17&&s.prop17) s.eVar17="D=c17";
  if(!s.eVar14&&s.pageName) s.eVar14="D=pageName";
  if(!s.eVar9&&s.prop10) s.eVar9="D=c10";
  if(!s.eVar23&&s.prop11) s.eVar23="D=c11";
...
/* * Plugin: omnitureReviews 0.6 - Depends on ratingsDisplayed function * defined before the bazaarvoice code (for review collection). */
...
// define helper function for multi touch point analysis
function s_uj(a,i) {
  return unescape(a[i][0]);
}
s.uj=s_uj;
...
s.vpr=new Function("vs","v", "var s=this,k=vs.substring(0,2)=='s_'?vs.substring(2):vs;s['vpv_'+k]="
+"v;s['vpm_'+k]=1");
s.dt=new Function("tz","t", "var d=new Date;if(t)d.setTime(t);d=new Date(d.getTime()+(d.getTimezo"
+"neOffset()*60*1000));return new Date(Math.floor(d.getTime()+(tz*60*"
+"60*1000)))");
s.vh_gt=new Function("k","v", "var s=this,vh='|'+s.c_r('s_vh_'+k),vi=vh.indexOf('|'+v+'='),ti=vi<0?"
+"vi:vi+2+v.length,pi=vh.indexOf('|',ti),t=ti<0?'':vh.substring(ti,pi"
+"<0?vh.length:pi);return t");
s.vh_gl=new Function("k", "var s=this,vh=s.c_r('s_vh_'+k),e=vh?vh.indexOf('='):0;return vh?(vh."
+"substring(0,e?e:vh.length)):''");
s.vh_s=new Function("k","v", "if(k&&v){var s=this,e=new Date,st=e.getTime(),y=e.getYear(),c='s_vh_"
...
s.visitorNamespace="tescostores"
s.dc="122"
s.trackingServer="metrics.tesco.com"
s.trackingServerSecure="smetrics.tesco.com"
